	...
	namespace Controller {
		MStatus RoutedMeshImporter::read(const char *filename) {
...

			{
                ...
                MVector tangent((normal ^ MVector::yAxis ^ normal).normal().rotateBy(boost::math::quaternion<double>(toRadians(rotation), normal)));
				...
				for (; it != m_edges.end(); ++it, ++prev_it) {
					...
					if (next_it != m_edges.end()) {
                        ...
                        const MVector end_base(end_cylinder + tangent.rotateBy(boost::math::quaternion<double>(toRadians(DNA::HelixRotation(length)), normal)));
						...
						if (next_it->hasAngle)
                            tangent = MVector((normal ^ MVector::yAxis ^ normal).normal().rotateBy(boost::math::quaternion<double>(toRadians(next_it->angle), normal)));
						else {
