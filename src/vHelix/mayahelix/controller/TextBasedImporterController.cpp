...
namespace Helix {
	namespace Controller {
        ...
        std::string getBaseFromConnectionType(Model::Helix & helix, TextBasedImporter::Connection::Type type, Model::Base & base) {
            std::string status;
			switch (type) {
        ...
        std::string TextBasedImporter::read(const char *filename, int nicking_min_length, int nicking_max_length) {
            std::string status;
            ...
            std::vector<double> orientation(4); // x, y, z, w
			unsigned int bases;
			...
			while (file.good()) {
                ...
                if (sscanf(line.c_str(), "h %s %lf %lf %lf %lf %lf %lf %lf", nameBuffer, &position[0], &position[1], &position[2], &orientation[0], &orientation[1], &orientation[2], &orientation[3]) == 8)
					helices.push_back(Helix(position, orientation, nameBuffer));
                else if (sscanf(line.c_str(), "hb %s %u %lf %lf %lf %lf %lf %lf %lf", nameBuffer, &bases, &position[0], &position[1], &position[2], &orientation[0], &orientation[1], &orientation[2], &orientation[3]) == 9)
			...
			for (std::vector<Helix>::iterator it(helices.begin()); it != helices.end(); ++it) {
				Model::Helix helix;
                QQuaternion transform(QVector4D(1, it->position[0], it->position[2], it->position[3]));
                QQuaternion rotation(QVector4D(it->orientation[3], it->orientation[0], it->orientation[1], it->orientation[2]));
