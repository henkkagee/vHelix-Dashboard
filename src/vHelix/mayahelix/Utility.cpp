...
namespace Helix {
	...
	MStatus HelixBase_RemoveAllAimConstraints(MObject & helixBase, const char *type) {
		...
		if (foundAimConstraint) {
            ...
            if (!(status = this_transform.setRotation(rotation, boost::numeric::ublas::matrix<double>::kXYZ))) {
	...
	double signedAngle(const MVector & from, const MVector & to, const MVector & normal) {
        boost::math::quaternion<double> rotation(from, to);
		MVector axis;
		double angle;
	...
	namespace Debug {
		void printf(const char *file, const char *function, size_t line, const char *expr, ...) {
			...
			fprintf(stdout, "%s(%lu): in %s: ", file, line, function);
			vfprintf(stdout, expr, vl);
			fprintf(stdout, "\n");
