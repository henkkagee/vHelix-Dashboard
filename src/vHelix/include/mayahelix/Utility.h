...
namespace Helix {
	...
	*/
	...
	namespace Debug {
		...
		std::string evaluate(const char *file, const char *function, size_t line, const char *expr, const std::string & status);
	}
...
#define HMEVALUATE_RETURN_DESCRIPTION(description, status) if (!status) { ::Helix::Debug::evaluate(__FILE__, __FUNCTION__, __LINE__, description, (status)); return status; }
	...
	std::string SelectedBases(MObjectArray & result);
	//std::string SelectedStrands(MObjectArray & strands, MObjectArray & bases);
	
	bool HelixBase_NextBase(const MObject & base, const MObject & attribute, MDagPath & result, std::string *retStatus = NULL);
	...
	//unsigned int HelixBase_endType(MObject & base, std::string *retStatus = NULL);

	//std::string Helix_Relatives(const MObject & helix, MObjectArray & result);

	//bool HelixBase_isForward(const MObject & base, std::string *retStatus = NULL);

	std::string HelixBases_sort(MObjectArray & input, MObjectArray & result);

	std::string HelixBase_RemoveAllAimConstraints(MObject & helixBase, const char *type = "aimConstraint");
	...
	std::string GetSelectedObjectsOfType(MObjectArray & objects, MTypeId & type);

	/*
	 * If object is of type `type` it is returned. If it's not, all its parents are searched
	 */

	MObject GetObjectOrParentsOfType(MObject & object, MTypeId & type, std::string & status);
	...
	/*
	 * In comparison to std::vector<double>::angle this also considers the sign when doing a rotation
	 * along the given normal axis.
	 */
	double signedAngle(const std::vector<double> & from, const std::vector<double> & to, const std::vector<double> & normal);
	 ...
	 * std::string (MSelectionList::* GetElementFunc) (unsigned int index, ElementT & element) const
	...
	std::string ArgList_ArgumentNodes(const MArgList & args, const MSyntax & syntax, const char *flag, ArrayT & array) {
		std::string status;
	...
	std::string ArgList_GetObjects(const MArgList & args, const MSyntax & syntax, const char *flag, MObjectArray & result);
	std::string ArgList_GetDagPaths(const MArgList & args, const MSyntax & syntax, const char *flag, MDagPathArray & result);

	/*
	 * We could work with the lists of helices/bases directly and save some performance
	 */

	template<typename ElementT, typename ArrayT, typename ContainerT, std::string (*GetElementsFunc) (const MArgList &, const MSyntax &, const char *, ArrayT &) >
	std::string ArgList_GetModelObjectsT(const MArgList & args, const MSyntax & syntax, const char *flag, ContainerT & result) {
		std::string status;
		ArrayT objects;
	...
	inline std::string ArgList_GetModelObjects(const MArgList & args, const MSyntax & syntax, const char *flag, ContainerT & result) {
	...
	std::string CVCurveFromObject(const Model::Object & object, MPointArray & array);
	...
	std::string SetupOpenGLShaders(const char **vertex_shader_source, const char **fragment_shader_source, const char **uniform_names, GLint *uniform_locations, GLsizei uniform_count, const char **attrib_names, GLint *attrib_locations, GLsizei attrib_count, GLuint & program, GLuint & vertex_shader, GLuint & fragment_shader);
	...
	*/
	inline std::string StringIdentifiersToObjects(const std::stringArray & strings, MObjectArray & objects) {
		std::string status;
		MSelectionList list;
	...
	inline std::string StringIdentifierToObject(const std::string & string, MObject & object) {
		std::string status;
		MSelectionList list;
