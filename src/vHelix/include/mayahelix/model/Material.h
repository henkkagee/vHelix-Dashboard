...
namespace Helix {
	namespace Model {
		...
		public:
			inline Material(const std::string & string) : m_material(string) {
			...
			inline Material & operator=(const std::string & string) {
				m_material = string;
			...
			static std::string Create(const std::string & name, float color[3], Material & material);

			static std::string Find(const std::string & name, Material & material);

			typedef std::vector<Material> Container;
			typedef const Material * Iterator;

			static Iterator AllMaterials_begin(std::string & status, Container::size_type & numMaterials);
			static Iterator AllMaterials_begin(std::string & status);
			
			static Iterator AllMaterials_end();

			std::string getColor(float color[3]) const;
