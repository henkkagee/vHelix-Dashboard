...
namespace Helix {
	namespace Controller {
		...
		public:
			...
			inline std::string createCurves(const MObjectArray & helices, const MObjectArray & bases, int degree) {
		...
		private:
			...
			struct Curve {
				...
				std::string create(CreateCurves & instance);
			};
			...
			std::string createCurves(std::list<Model::Helix> & helices, std::list<Model::Strand> & strands);
			std::string createCurve(Model::Strand & strand);
		};
	}
}
